...
export function CustomerSatisfaction({
  ...
  // Custom Tooltip for Customer Satisfaction Distribution
  const CustomSatisfactionTooltip = ({ active, payload }: any) => {
  ...
  // Custom Tooltip for Satisfaction vs Retention Correlation
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div style={{ height: '300px' }}>
              <DonutChart
                data={satisfactionData.starRatings.map(rating => ({
                  id: rating.label.toLowerCase().replace(' ', '-'),
                  name: rating.label,
                  value: rating.customers,
                  color: rating.color,
                  percentage: rating.percentage,
                  stars: rating.stars,
                  satisfaction: rating.satisfaction,
                  customers: rating.customers,
                  avgSpend: rating.avgSpend,
                  retentionRate: rating.retentionRate,
                  description: `${rating.satisfaction} - Avg Spend: ${getFormattedValue(rating.avgSpend)}, Retention: ${rating.retentionRate}%`,
                }))}
                ...
                customTooltip={CustomSatisfactionTooltip}
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div style={{ height: '300px' }}>
              <BarChartComponent
                data={satisfactionData.starRatings.map(rating => ({
                  name: rating.label,
                  value: rating.retentionRate,
                  color: rating.color,
                  customers: rating.customers,
                  avgSpend: rating.avgSpend,
                  retentionRate: rating.retentionRate,
                }))}
                valueFormatter={value => `${value}% retention`}
                valueUnit="%"
                customTooltip={CustomRetentionTooltip}
                onBarClick={data => console.log('Clicked bar:', data)}
              />
            </div>
            {/* Legend */}
            <div className="space-y-2 mt-4">
